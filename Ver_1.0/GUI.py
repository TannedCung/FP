    ...
    def VS(self):
        ...
        while True :
            ...
            for i, faceID in enumerate(self.faceTracker.keys()):
                counter = 0 # count the nummber of face passed to face_pics(infor in info_frame)
                trackedPosition = self.faceTracker[faceID].get_position()
                ...
                # no more recognition
                if self.faceStatus[faceID] == 6: # a stranger
                    frame = self.the_bodyguard.draw_label(frame=frame, point=self.faceCurrentPos[faceID],
                                            score=self.score[faceID], flag=6, label=self.label[faceID])
                # if the face state is Known, draw that person information
                elif self.faceStatus[faceID] == 5: # an acquantance
                    if counter<=3 and t_x>0 and t_y>0:
                        save = frame[t_y:t_y+t_h, t_x:t_x+t_w]
                        face_pics[counter] = cv2.resize(save, (100,100))
                        info[counter] = self.new_std.search_info(name=self.label[faceID])

                    frame = self.the_bodyguard.draw_label(frame=frame, point=self.faceCurrentPos[faceID],
                                            score=self.score[faceID], flag=5, label=self.label[faceID])
                ...
                # that person should be a stranger
                    ...
                    if not (frame_counter % 2):
                        ...
                        face = np.expand_dims(face, axis=0)
                        name, score = self.the_bodyguard.identify_face(face)
                        self.score[faceID] = (self.score[faceID]*self.faceStatus[faceID]+score)/(self.faceStatus[faceID]+1)
                    ...
                    frame = self.the_bodyguard.draw_label(frame=frame, point=self.faceCurrentPos[faceID],
                                                score=self.score[faceID], flag=self.faceStatus[faceID]-1 if self.faceStatus[faceID]<5 else 4) 
